...
const { uploadProfilePhotoToS3 } = require('../services/s3.service');

const ALLOWED_MIME_TYPES = new Set(['image/jpeg', 'image/png']);
const MAX_FILE_SIZE_BYTES = 5 * 1024 * 1024;
const ALLOWED_FIELDS = new Set(['photo', 'image']);

const uploadProfilePhoto = async (req, _res, next) => {
  ...
  busboy.on('file', (fieldname, fileStream, info) => {
    ...
    if (!ALLOWED_FIELDS.has(fieldname)) {
      fileStream.resume();
      fail(new ApiError(StatusCodes.BAD_REQUEST, 'Unexpected file field; expected "photo"'));
      return;
    }
    ...
    fileProcessingPromise = uploadProfilePhotoToS3(passThrough, filename, mimeType, userId)
      .then((url) => {
  ...
  busboy.on('finish', async () => {
    ...
    try {
      ...
      if (!fileSeen) {
        req.uploadedProfilePhotoUrl = null;
        return done();
      }

      req.uploadedProfilePhotoUrl = uploadedPhotoUrl;
      return done();
    } catch (_error) {
